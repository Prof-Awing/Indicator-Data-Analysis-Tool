    ...
    while True:
        try:
            ...
            if key==1:
                ...
                while True:
                    apikey= input("Input your API-Key from TradingEconomics account: ").strip()
                    try:
                        te.login(apikey)
                        account_type= key
                        break
...
def column_select(json_dict_list):
    ...
    while True:
        try:
            ...
            digits=[]
            for i in n:
                if (len(n)==1 and n[0]=='all') or (int(i) in pairs.keys() and 0 <= int(i) <= 16):
                    digits.append(i)
                else:
                    print("Outside the provided options or range of integers(keys). Try again.")
                    continue
            break
        except ValueError:
...
def customize(dataframe):
    while True:
        y= input("Want to narrow down or customize the table visualization (Recommended if your data is so complex)? y/n: ")
        if y=="y":
            while True:
                print(f'''
...
def solid_or_percentage_increase_from_previous_value(df):
    while True:
        y_n= input("Do you want to calculate the increase in Value from the previous? y/n?: ")
        if y_n.strip()=="y":
            df['Value Change']= df['LatestValue'] - df['PreviousValue']
            df['Percentage Change(%)']= ((df['Value Change'] / df['PreviousValue'])*100).round(2)
            print(df)
            tf= True
            while True:
                if "Category" in selected_columns and "Country" in selected_columns and len(countries) > 1:
                    ...
                    if quiz.strip()=="y":
                        ...
                        gg= choosen_catdf[abs(choosen_catdf['Value Change'])== max_val]
                        c= [i for i in gg['Country']]
                        while True:
                            x= input(f"Which country shows the highest change in value (Value Change) in {r_ind} category?: ")
                            if x.strip().title() in c:
                                print(f"Correct! It's {x.title()}")
                                break
                            else:
                                try_= input("Oops! Not quite.\n Try again(t) or Reveal answer(r)? t/r: ")
                                if try_.strip() == "t":
                                    continue
                                elif try_.strip()== "r":
                                    print("The answer is/are:", revealor(c), "\nThank you!")
                                else:
...
def main():
    ...
    while True:
        ...
        if indicator!=[]:
            ...
            continue
    data= te.getIndicatorData(country= countries, indicators= indicator, output_type= "dict")
    if data is not None:
        ...
        sys.exit("No data available for the provided combination of Country and Indicator. Please try again.")
